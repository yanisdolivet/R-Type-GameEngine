                                 ...
                                 SparseArray<Components::Collider> const& colliders) const
{
    for (auto&& [idxA, posA, colliderA] : IndexedZipper(positions, colliders)) {
        for (auto&& [idxB, posB, colliderB] : IndexedZipper(positions, colliders)) {
            ...
            bool collisionX =
                posA.getX() < posB.getX() + colliderB.getWidth() * scaleB.getX() && posA.getX() + colliderA.getWidth() * scaleA.getX() > posB.getX();
            bool collisionY =
                posA.getY() < posB.getY() + colliderB.getHeight() * scaleB.getY() && posA.getY() + colliderA.getHeight() * scaleA.getY() > posB.getY();
